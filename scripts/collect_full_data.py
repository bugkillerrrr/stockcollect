...
class FullDataCollector:
    ...
    def __init__(self):
        ...
        self.collector = AkshareDataCollector(request_interval=6.0, max_retries=3)
        self.db = StockDataDB()
        ...
        self.progress_file = self.log_dir / "collection_progress.json"
    ...
    def get_pending_stocks(self, all_stock_codes):
        ...
        try:
            # 获取已完成数据收集的股票代码
            completed_codes = set(self.db.get_completed_stock_codes())
    ...
    def collect_stock_data(self, stock_code, start_date=None, end_date=None):
        ...
        try:
            ...
            if data is not None and not data.empty:
                ...
                if count > 0:
                    self.logger.info(f"成功收集 {stock_code} 数据: {count} 条记录")
                    return True
                else:
                    self.logger.warning(f"{stock_code} 数据保存失败")
                    return False
            else:
                self.logger.warning(f"{stock_code} 没有获取到数据")
                return False
    ...
    def collect_all_data(self, stock_codes=None, max_retries=3, delay=1.0, limit=None):
    ...
    def get_database_stats(self):
        ...
        try:
            ...
            self.logger.info(f"  技术数据记录: {stats.get('technical_data_records', 0):,}")
            self.logger.info(f"  股票主表记录: {stats.get('stock_master_records', 0):,}")
            self.logger.info(f"  数据库大小: {stats.get('database_size_mb', 0):.2f} MB")
        except Exception as e:
            self.logger.error(f"获取数据库统计信息失败: {e}")

def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='股票数据系统 - 完整数据收集程序')
    ...
    if not args.auto:
        try:
            response = input("是否开始收集完整数据? (y/N): ").strip().lower()
    ...
    print(f"\n{mode_names[args.mode]}: {mode_names[args.mode]}")
