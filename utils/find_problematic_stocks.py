...
def find_problematic_stocks(db_path: str = "data/stock_data.db"):
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            # 分类问题股票
           严重数据不足 = stock_stats[stock_stats['record_count'] < 10]
            数据较少 = stock_stats[(stock_stats['record_count'] >= 10) & (stock_stats['record_count'] < 50)]
            数据偏少 = stock_stats[(stock_stats['record_count'] >= 50) & (stock_stats['record_count'] < 200)]

            print(f"\n=== 问题股票分类 ===")
            print(f"严重数据不足(<10条): {len(严重数据不足)} 只")
            print(f"数据较少(10-49条): {len(数据较少)} 只")
            print(f"数据偏少(50-199条): {len(数据偏少)} 只")

            # 详细列出严重问题的股票
            if len(严重数据不足) > 0:
                print(f"\n=== 严重数据不足的股票 ({len(严重数据不足)}只) ===")
                for _, row in 严重数据不足.iterrows():
            ...
            # 列出数据较少的股票
            if len(数据较少) > 0:
                print(f"\n=== 数据较少的股票 ({len(数据较少)}只，前20只) ===")
                for _, row in 数据较少.head(20).iterrows():
                    print(f"  {row['stock_code']}: {row['record_count']} 条记录 ({row['earliest_date']} 到 {row['latest_date']})")

                if len(数据较少) > 20:
                    print(f"  ... 还有 {len(数据较少) - 20} 只未显示")
            ...
            with open(report_file, 'w', encoding='utf-8') as f:
                ...
                f.write("问题股票统计:\n")
                f.write(f"1. 严重数据不足(<10条): {len(严重数据不足)} 只\n")
                f.write(f"2. 数据较少(10-49条): {len(数据较少)} 只\n")
                f.write(f"3. 数据偏少(50-199条): {len(数据偏少)} 只\n")
            ...
            # 返回需要重点关注的股票代码
            critical_stocks = 严重数据不足['stock_code'].tolist() + 数据较少['stock_code'].tolist()
